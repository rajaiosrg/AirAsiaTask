...
public class SVPinView: UIView {
    ...
    public var shouldShowKeboard : Bool = true
    ...
    @objc
    public func currentPasswordLength() -> Int {
    ...
    public func enterNumberAtIndex(at index : Int , text : String) {
        let indexPath = NSIndexPath(row: index, section: 0) as IndexPath
        let collectionCell = collectionView.cellForItem(at: indexPath)
        
        let containerView = collectionCell?.viewWithTag(51)!
        let cellSubViews = containerView?.subviews
        for subView in cellSubViews! {
            if subView .isKind(of: SVPinField.self) {
                let textField = subView as! SVPinField
                textField.text = text
...
extension SVPinView : UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout
{
    ...
    public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        textField.keyboardType = UIKeyboardType.webSearch //self.keyboardType
